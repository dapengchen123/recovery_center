...
def avepool_dismat(distmat,  query_ids=None, gallery_ids=None,
            query_cams=None, gallery_cams=None):
    ### for video sequence evaluation   --dapeng
    ### using average images score to represent the score of two sequences --dapeng
    ...
    query_images = np.transpose(np.vstack((query_ids, query_cams)))
    gallery_images = np.transpose(np.vstack((gallery_ids, gallery_cams)))

    single_query = []
    [single_query.append(tuple(r)) for r in query_images if tuple(r) not in single_query]
    query_seq = np.array(single_query)

    single_gallery = []
    [single_gallery.append(tuple(r)) for r in gallery_images if tuple(r) not in single_gallery]
